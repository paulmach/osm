...
func TestChangesetScanner(t *testing.T) {
	r := changesetReader()
	scanner := NewChangesetScanner(context.Background(), r)

	if v := scanner.Scan(); v == false {
		t.Fatalf("should read first scan: %v", scanner.Err())
	}

	if cs := scanner.Changeset(); cs.ID != 41226352 {
	...
	if cs := scanner.Changeset(); cs.ID != 41227987 {
		t.Fatalf("did not scan correctly, got %v", cs)
	}
...
func TestChangesetScannerContext(t *testing.T) {
	...
	scanner := NewChangesetScanner(ctx, r)
...
func TestChangesetScannerErr(t *testing.T) {
	...
	scanner := NewChangesetScanner(nil, r)
